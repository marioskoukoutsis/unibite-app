...
// --- Εμφάνιση Admin Portal στο Navigation αν είναι Admin ---
    ...
    window.deleteListing = async function(id) {
        ...
        try {
            ...
            if (response.ok) fetchMyListings();
    ...
    async function fetchMyRequests() {
        ...
        try {
            ...
            container.innerHTML = '<p style="color: red;">Αποτυχία φόρτωσης αιτημάτων.</p>';
        }
    }
    ...
    function renderRequests(requests, container) {
        ...
        if (requests.length === 0) {
            container.innerHTML = '<p style="color: var(--text-muted);">Δεν υπάρχουν αιτήματα ακόμα.</p>';
            return;
        }
        ...
        requests.forEach(req => {
            ...
            const dateStr = new Date(req.created_at).toLocaleString('el-GR', { dateStyle: 'medium', timeStyle: 'short' });
            const pickupStr = new Date(req.pickup_time).toLocaleString('el-GR', { dateStyle: 'medium', timeStyle: 'short' });
            const color = statusColors[req.status] || '#6b7280';
            const label = statusLabels[req.status] || req.status;
            const ratingHtml = req.rating ? `<span style="margin-left: 0.5rem; color: #f59e0b; font-size: 0.85rem;">| Βαθμολογία: ${req.rating}/5</span>` : '';

            let actionsHtml = '';
            if (req.status === 'pending') {
                actionsHtml = `
                    <div style="display: flex; gap: 8px; margin-top: 1rem;">
                        <button class="btn" style="flex:1; padding: 0.5rem;" onclick="updateRequestStatus(${req.id}, 'approved')">Αποδοχή</button>
                        <button class="btn" style="flex:1; padding: 0.5rem; background: transparent; color: #ef4444; border: 1px solid #fca5a5; box-shadow: none;" onclick="updateRequestStatus(${req.id}, 'rejected')">Απόρριψη</button>
                    </div>`;
            } else if (req.status === 'approved') {
                actionsHtml = `
                    <div style="display: flex; gap: 8px; margin-top: 1rem;">
                        <button class="btn" style="flex:1; padding: 0.5rem; background: #10b981;" onclick="updateRequestStatus(${req.id}, 'picked_up')">Παραλήφθηκε</button>
                        <button class="btn" style="flex:1; padding: 0.5rem; background: transparent; color: #6b7280; border: 1px solid #d1d5db; box-shadow: none;" onclick="updateRequestStatus(${req.id}, 'no_show')">Δεν παραλήφθηκε</button>
                    </div>`;
            }
            ...
            card.innerHTML = `
                <div style="display: flex; justify-content: space-between; align-items: start;">
                    <div style="flex: 1;">
                        <p style="margin: 0 0 0.3rem; font-weight: 700; color: var(--text-main);">${req.consumer_name}</p>
                        <p style="margin: 0 0 0.3rem; color: var(--text-muted); font-size: 0.9rem;">${req.listing_title}</p>
                        <p style="margin: 0 0 0.2rem; color: var(--text-muted); font-size: 0.85rem;">Παραλαβή: ${req.pickup_location} | ${pickupStr}</p>
                        <p style="margin: 0; color: var(--text-muted); font-size: 0.85rem;">Αίτημα: ${dateStr}${ratingHtml}</p>
                    </div>
                    <span style="background: ${color}20; color: ${color}; border: 1px solid ${color}40; border-radius: 999px; padding: 0.2rem 0.8rem; font-size: 0.85rem; white-space: nowrap; margin-left: 1rem;">${label}</span>
                </div>
                ${actionsHtml}
            `;
